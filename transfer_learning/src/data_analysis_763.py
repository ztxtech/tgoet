...
# 设置字体大小
plt.rcParams['font.size'] = 14  # 标签和刻度的字体大小
...
# 将数据转换为 DataFrame
df = pd.DataFrame(data, columns=['Feature1', 'Feature2', 'Feature3', 'Class'])

plt.figure(figsize=(12, 9), dpi=500)  # 增加图片尺寸
# 使用 seaborn 的 pairplot 绘制散点图矩阵
sns.pairplot(df, hue='Class', palette='tab10', markers=['o', 's', 'D', '^', 'P'])  # 指定颜色方案和标记样式

plt.savefig("./cache/mask/763.pdf", bbox_inches='tight')
